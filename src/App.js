...
const ListProvisioningGateways = React.lazy(() =>
  import("./provisioning/ListProvisioningGateways")
);
  ...
  <Navbar staticTop collapseOnSelect inverse>
    ...
    <Navbar.Collapse>
      <Nav>
            ...
            >
              ...
              {supportedModule(modules.npact_crdb, user_info.modules) && (
                <LinkContainer to={"/transactions/new_install_address"}>
                  <MenuItem>
                    <FormattedMessage
                      id="new-addess"
                      defaultMessage="New Address Change"
                    />
                  </MenuItem>
                </LinkContainer>
              )}
                ...
                <LinkContainer to={"/transactions/emergency_notification"}>
                  <MenuItem>
                    <FormattedMessage
                      ...
                      defaultMessage="New Emergency Notification"
                    />
                  </MenuItem>
                </LinkContainer>
              )}
              ...
              <LinkContainer to={"/transactions/list"}>
                <MenuItem>
                  <FormattedMessage
                    id="porting-requests"
                    defaultMessage="Porting Requests"
                  />
                </MenuItem>
              </LinkContainer>
            </NavDropdown>
          ...
          isAllowed(user_info.ui_profile, pages.requests_nprequests) && (
            ...
            >
              ...
              {(!user_info.modules ||
                ...
                <LinkContainer
                  to={"/custom-transactions/list"}
                  key="custom-requests"
                >
                  <MenuItem>
                    <FormattedMessage
                      id="custom-requests"
                      defaultMessage="Custom Requests"
                    />
                  </MenuItem>
                </LinkContainer>,
              ...
              {(!user_info.modules ||
                user_info.modules.includes(modules.orange)) &&
                isAllowed(user_info.ui_profile, pages.requests_ndg) && [
                  <MenuItem key="divider-1" divider />,
                  <LinkContainer key="ndg-history" to={"/requests/ndg"}>
                    <MenuItem>
                      <FormattedMessage
                        id="ndg-history"
                        defaultMessage="NDG history"
                      />
                    </MenuItem>
                  </LinkContainer>
                ]}
        ...
        {(!user_info.modules || user_info.modules.includes(modules.bulk)) &&
          isAllowed(user_info.ui_profile, pages.bulks) && (
            <NavDropdown
              title={
                <span>
        ...
        {(!user_info.modules ||
          user_info.modules.includes(modules.provisioning)) &&
          isAllowed(user_info.ui_profile, pages.provisioning) && (
            <NavDropdown
              eventKey={4}
              title={
                <span>
        ...
        {(!user_info.modules ||
          supportedModule(modules.npact, user_info.modules)) &&
          isAllowed(user_info.ui_profile, pages.data) && (
            <NavDropdown
              eventKey={4}
              title={
                <span>
              ...
              {isAllowed(user_info.ui_profile, pages.npact_porting_cases) && (
                <LinkContainer to={"/system/porting_cases"}>
                  <MenuItem>
                    <FormattedMessage
                      id="np-database"
                      defaultMessage="NP database"
                    />
                  </MenuItem>
                </LinkContainer>
              )}
              {user_info.modules.includes(modules.npact_crdc) &&
                isAllowed(user_info.ui_profile, pages.npact_mvno_numbers) && (
                  <LinkContainer to={"/system/mvno_numbers"}>
                    <MenuItem>
                      <FormattedMessage
                        id="mvno-numbers"
                        defaultMessage="MVNO Numbers"
                      />
                    </MenuItem>
                  </LinkContainer>
                )}
        ...
        {(!user_info.modules ||
          ...
          isAllowed(user_info.ui_profile, pages.requests_startup_events) && (
            ...
            >
              {isAllowed(
                user_info.ui_profile,
                pages.requests_startup_events
              ) && (
                <LinkContainer to={"/transactions/config/startup_events"}>
                  <MenuItem>
                    <FormattedMessage
                      id="startup-events"
                      defaultMessage="Startup Events"
                    />
                  </MenuItem>
                </LinkContainer>
              )}
              {isAllowed(
                user_info.ui_profile,
                pages.requests_workflow_editor
              ) && (
...
const DemoWatermark = () => (
  <>
    ...
    >
      This version is not fully activated (only for DEMO)! Contact
      support@netaxis.be for full activation.
    </div>
    <div
      style={{
        position: "fixed",
  ...
  if (validUntil < new Date().addDays(30) && validUntil >= new Date()) {
    return (
      <Alert bsStyle="warning">
        This instance run a {health.demo ? "demo " : ""}license which will
        expire soon ({validUntil.toLocaleString()}) - Take contact with
        support@netaxis.be before it's too late.
      </Alert>
    );
  } else if (validUntil < new Date()) {
    return (
      <Alert bsStyle="danger">
        This instance run an invalid license - Take contact with
        support@netaxis.be to activate your instance.
      </Alert>
    );
  } else {
    return watermark;
...
class App extends Component {
  ...
  getUserInfo() {
    fetch_get("/api/v01/system/users/local")
      .then(data => {
        ...
        if (
          data.modules &&
          supportedModule(modules.provisioning, data.modules)
        ) {
          import("./provisioning").then(prov => {
            this.setState({
              provisioningRoutes: prov.provisioningRoutes(data.ui_profile)
            });
          });
          ProvProxiesManager.fetchConfiguration()
            .then(() => this.setState({ proxy_fetch: true }))
            .catch(console.log);
        }
      })
      .catch(error => {
  ...
  componentWillUpdate() {
    if (
      ...
      AuthServiceManager.isAuthenticated()
    ) {
      this.getUserInfo();
    }
  }
  ...
  componentDidMount() {
    ...
    getCookie("auth_sso") === "1" &&
      !sso_auth_service.isLoggedIn() &&
      sso_auth_service.signinSilent();
  }
  ...
  updateToken(token, sso_auth) {
    ...
    user_info.modules &&
      supportedModule(modules.provisioning, user_info.modules) &&
      ProvProxiesManager.fetchConfiguration()
        .then(() => this.setState({ proxy_fetch: true }))
        .catch(console.log);
  ...
  logout() {
    ...
    sso_auth_service
      .removeUser()
      .then(() => this.props.onLanguageUpdate(undefined));
  }
  ...
  isAuthenticated() {
    ...
    if (localStorage.getItem("auth_sso")) {
      ...
      sso_auth_service.manager.events.addUserLoaded(
        this.ssoTokenToLocalToken.bind(this)
      );
    }
    const sso_auth = sso_auth_service.isLoggedIn();

    console.log(
      `local_auth status: ${local_auth}, sso_auth status: ${sso_auth}`
    );
          ...
          <Col mdOffset={1} md={10}>
            <Suspense fallback={<div>Loading...</div>}>
              <Switch>
                <Route
                  path="/help"
                  component={props => <AsyncApioHelp {...props} />}
                  exact
                />
                <Route
                  path="/dashboard"
                ...
                <Route
                  path="/transactions/config/startup_events"
                  component={props =>
                ...
                />
                <Route
                  path="/transactions/config/activities/editor"
                  component={props =>
                ...
                />
                <Route
                  path="/transactions/config/activities/editor/:activityId"
                  component={props =>
                    isAllowed(ui_profile, pages.requests_workflow_editor) ? (
                ...
                />
                <Route
                  path="/transactions/config/cron_timers"
                  component={props =>
                    isAllowed(ui_profile, pages.requests_workflow_editor) ? (
                      <CronTimers />
                    ) : (
                ...
                />
                <Route
                  path="/transactions/mobile_events"
                  component={props =>
                    isAllowed(ui_profile, pages.requests_nprequests) ? (
                      <MobileEventsManagement
                        auth_token={auth_token}
                        {...props}
                      />
                    ) : (
                ...
                />
                <Route
                  path="/transactions/new_disconnect"
                  component={props =>
                    isAllowed(ui_profile, pages.requests_nprequests) ? (
                      <NPDisconnectRequest
                        auth_token={auth_token}
                ...
                />
                <Route
                  path="/transactions/new_install_address"
                  component={props =>
                    isAllowed(ui_profile, pages.requests_nprequests) ? (
                      <NPChangeInstallationAddressRequest {...props} />
                    ) : (
                      <NotAllowed />
                    )
                  }
                ...
                />
                <Route
                  path="/transactions/emergency_notification"
                  component={() =>
                    isAllowed(ui_profile, pages.requests_nprequests) ? (
                ...
                <Route
                  ...
                  component={ListProvisioningGateways}
                  exact
                />
                {
