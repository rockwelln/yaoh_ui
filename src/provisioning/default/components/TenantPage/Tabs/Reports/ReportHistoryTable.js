...
import { fetchGetReportingCustomerReportHistory } from "../../../../store/actions";
...
function ReportHistoryTable({ tenantId }) {
  ...
  useEffect(() => {
    if (tenantId) {
      dispatch(fetchGetReportingCustomerReportHistory(tenantId)).then(() =>
        setIsLoading(false)
      );
    }
  }, [tenantId]);
