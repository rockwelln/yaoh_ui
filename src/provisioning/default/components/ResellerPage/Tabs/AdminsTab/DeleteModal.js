...
import { fetchDeleteResellerAdmin } from "../../../../store/actions";

import { FormattedMessage } from "react-intl";

const DeleteModal = ({ adminId, show, onClose }) => {
  const params = useParams();
  const dispatch = useDispatch();
  const [deleting, setDeleting] = useState(false);
  const onDelete = () => {
    setDeleting(true);
    dispatch(fetchDeleteResellerAdmin(params.resellerName, adminId)).then(() =>
      onClose("deleted")
    );
  };
