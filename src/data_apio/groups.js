...
class DetailsModal extends Component {
    ...
    loadDetails(tenantId, groupId) {
        fetch_get(`${API_URL_PROXY_PREFIX}/api/v1/orange/tenants/${tenantId}/groups/${groupId}/`, this.props.auth_token)
            .then(data => this.setState({data: data.group}))
            .catch(error => {
                this.setState({deleting: false});
                this.props.notifications.addNotification({
                    title: <FormattedMessage id="fetch-group-fail" defaultMessage="Fail fetch group"/>,
                    level: 'error',
                })
            })
    }
                ...
                <Modal.Body>
                    {
                        data ? (
                            <Form horizontal>
                                ...
                                <FormGroup>
                                    <Col componentClass={ControlLabel} sm={2} md={3}>
                                        <FormattedMessage id='trunk-capacity' defaultMessage='Trunk capacity' />
                                    </Col>
...
class DeleteModal extends Component {
    ...
    onDelete(groupId) {
        ...
        fetch_delete(`${API_URL_PROXY_PREFIX}/api/v1/orange/tenants/${tenantId}/groups/${groupId}?deleteNonEmpty=${deleteNonEmpty?1:0}`, auth_token)
            .then(() => {
                this.props.notifications.addNotification({
        ...
        return (
            <Modal show={show} onHide={() => onClose && onClose(false)} backdrop={false}>
                ...
                <Modal.Body>
                    ...
                    <p><FormattedMessage id="confirm-delete-warning" defaultMessage={`You are about to delete the group ${groupId}!`}/></p>
                ...
                <Modal.Footer>
                    <Button onClick={() => this.onDelete(groupId)} bsStyle="danger" disabled={deleting}>
...
class Group extends Component {
    ...
    render() {
        ...
        return (
            <tr key={g.groupId}>
                <td><Link to={`/apio/tenants/${tenantId}/groups/${g.groupId}/numbers`}>{g.groupId}</Link></td>
                <td>{g.groupName}</td>
                <td>
                    <ButtonToolbar>
