...
import { fetchDeleteGroupDevice } from "../../../../store/actions";
...
class DeleteModal extends Component {
  ...
  onDelete(deviceName) {
    const { onClose, tenantId, groupId } = this.props;
  ...
  render() {
    const { deviceName, show, onClose } = this.props;
        ...
        <Modal.Body>
          ...
          <p>
            <FormattedMessage
              ...
              defaultMessage={`You are about to delete the device ${deviceName}!`}
            />
          </p>
        </Modal.Body>
        <Modal.Footer>
          <Button onClick={() => this.onDelete(deviceName)} bsStyle="danger">
