...
export const TxTable = ({tx, request, userInfo, activities}) => (
    <Table condensed>
        <tbody>
            ...
            <tr><th><FormattedMessage id="owner" defaultMessage="Owner" /></th><td>{tx.username || request?.owner}</td></tr>
...
export class Requests extends Component{
    ...
    render() {
        ...
        const request_entities = user_info.modules?.includes(modules.orange) ? "request_entities" : "requests";
        const proxy_activated = user_info.modules?.includes(modules.proxy);
        const draasActivated = user_info.modules?.includes(modules.draas);
        const manualActions = user_info.modules?.includes(modules.manualActions);
                    ...
                    <Panel.Body collapsible>
                        <Form horizontal>
                            ...
                            <FormGroup>
                                ...
                                <Col sm={8}>
                                    <FormControl componentClass="input" value={filter_criteria.request_status.value}
                                        onChange={e => this.setState({
                                            filter_criteria: update(filter_criteria,
...

                            {
                                ...
                                <FormGroup>
                                    ...
                                    <Col sm={8}>
                                        <Select
                                          isClearable
                                          value={{value: filter_criteria.request_method.value, label: filter_criteria.request_method.value}}
                                          name="tenant"
                                          options={["get", "put", "post", "delete"].map(k => ({value: k, label: k}))}
                                          onChange={v => {
                                            this.setState({
                                              filter_criteria: update(this.state.filter_criteria,
                                                { request_method: { $merge: { value: v ? v.value : "" } } })
                                            })
                                          }}
                                          className="basic-select"
                                          classNamePrefix="select" />
